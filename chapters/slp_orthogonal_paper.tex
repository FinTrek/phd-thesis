...
% \subsubsection{Stop-loss premiums for reinsurance agreements}
...
%\mathbb{E}\left\{\min[(S_N-c)_{+},d]\right\}=\Exp\left[(S_N-c)_{+}\right]-\Exp\left[(S_N-c-d)_{+}\right]
%\end{equation}
...
% &= 1 + \frac{1}{cT} \Exp\Big[ \min\{S_{N(T)} - cT, 0 \} \Big] \nonumber \\
% &= 1 + \frac{1}{cT} \Exp\Big[ (S_{N(T)} - cT) - \max(S_{N(T)} - cT, 0 ) \Big] \nonumber \\
% &= (cT)^{-1} \Big[ \Exp (S_{N(T)} ) - \Exp (S_{N(T)} - cT)_+ \Big]\\
% &= 1 + \frac{\Exp[N(T)] \Exp[U_1]}{cT} - \frac{\Exp\Big[ (S_{N(T)} - cT)_+ \Big]}{cT} \nonumber \\
&= (cT)^{-1} \{ \Exp[N(T)] \, \Exp[U_1] - \Pi_{cT}(S_{N(T)}) \} \,.
% &=1 - \Exp\left[\left(1-\frac{S_{N(T)}}{cT}\right)_{+}\right]
\end{align}
...
\subsubsection{Approximating densities of positive random variables} \label{ssec:PolynomialApproxPositive}
...
% w(x)\defeq\gamma(r,m,x)=\frac{x^{r-1}\e^{-\frac{x}{m}}}{\Gamma(r)m^{r}},\text{ }x\in\RL^{+},
% \end{equation*}
...
% &= \sum_{k=0}^{\infty}  a_k \left( \frac{\Gamma(k+r)\Gamma(k+1)}{\Gamma(r)} \right)^{1/2} \sum_{i=0}^{k} (-1)^{k+i} \frac{\gamma(r+i,m,x)}{\Gamma(k-i+r)\Gamma(i+1)}
\end{align}
where
\begin{equation}\label{eq:PiExpression}
c_i
% = \frac{(-1)^i }{i! \sqrt{\Gamma(r)}} \sum_{k=i}^\infty a_k (-1)^k \frac{\sqrt{k! \Gamma(k+r) }} {\Gamma(k-i+1) }
...
% \begin{align*}
% c_i
% &= d_i m^i \frac{\Gamma(r+i)}{\Gamma(r)} \\
% &= m^i \frac{\Gamma(r+i)}{\Gamma(r)} \frac{(-1)^i \Gamma(r)^{\frac12} }{m^i \Gamma(r+i) \Gamma(i+1)} \sum_{k=i}^\infty a_k (-1)^k \frac{ \big[ \Gamma(k+r) \Gamma(k+1) \big]^{\frac12}}{\Gamma(k-i+r) } \\
% &= \frac{(-1)^i }{\Gamma(r)^{\frac12} \Gamma(i+1)} \sum_{k=i}^\infty a_k (-1)^k \frac{ \big[ \Gamma(k+r) \Gamma(k+1) \big]^{\frac12}}{\Gamma(k-i+r) }
% \end{align*}
...
% &= \sum_{k=i}^\infty a_k \text{Coefficient}\left[x^i, (-1)^{k} \binom{k + r - 1}{k}^{-\frac12} L_{k}^{r-1}\big( \frac{x}{m} \big) \right] \\
% &= \sum_{k=i}^\infty a_k (-1)^{k} \binom{k + r - 1}{k}^{-\frac12} \text{Coefficient}\left[x^i, \sum_{j=0}^{k} \binom{k + r - 1}{k - j} \frac{(-x/m)^j}{j!} \right] \\
%&
...
%Therefore, if we truncate  $\{ a_k \}_{k\in\N}$ so that $a_k = 0$ for $k > K \in \N$, and if the resulting $c_i$'s are non-negative, then \eqref{eq:PseudoMixtureOfGammas} tells us that the orthogonal polynomial approximation is a mixture of gamma distributions. This simplifies calculating survival functions, and stop-loss premiums.
...
%F_{S_N}(x)&=&f_N(0)+\sum_{n=0}^{\infty}\sum_{i=0}^{n}a_n\tilde{q}_{i,n}\Gamma_{l}(r+i,m,x) \quad \text{for } x \ge 0 \,.
%\end{eqnarray}
...
% First, we need to introduce the binomial distribution denoted by $\BinomialDist(n,p)$ with pmf
%  \[ f_N(k) = \binom{n}{k} p^k q^{n-k} \,, \quad \text{ for }k = 0,1,\dots, n\,, \]
 % where $p \in (0,1)$, $n \in \NL$, and $p+q=1$.
...
% \begin{align*}
% \Ftail_{S_N}(x) &= \sum_{i=1}^\alpha \binom{\alpha}{i} p^i q^{\alpha-i} \e^{-p \beta x} \sum_{j=0}^{i-1} \frac{(p \beta x)^j}{j!}
...
% \Exp\left[\left(S_N-a\right)_{+}\right] = \sum_{i=1}^\alpha \binom{\alpha}{i} p^i q^{\alpha-i} \left[ \frac{i}{p \beta} \Gamma_u\big(i+1, a p \beta, (a p \beta)^2 \big) - a \Gamma_u(i, \frac{1}{p \beta} , a) \right]
% \end{equation*}
% \begin{color}{red}
\begin{equation*}\label{eq:StopLossCompoundNegBinExp}
...
% We begin by stating some useful transform relations, then discuss the general Laplace inversion framework that we will use, and will apply the method to the compound distribution problem.
...
% \[ \Prob(X^\ast > d) = \frac12 + \frac1\pi PV \int_0^\infty \Re \left[ \frac{\e^{-i u d} \hat{\mu}_{X^\ast}(u)}{i u} \right] \dd u \]
% where $\hat{\mu}_{X^\ast}(u) = \frac{\hat{\mu}_X(u) - 1}{i u \Exp X}$.
...
%The approximation of limited stop-loss premium follows from the one of the stop-loss premium by taking advantage of the identity
%\begin{equation}\label{eq:LinkStopLossPremiumLimitedStopLossPremium}
%\Exp\left\{
%\min\left[\left(X-d\right)_{+},c\right]\right\}=\Exp\left\{
%\left[X-(c+d)\right]_{+}\right\}-\Exp\left[\left(X-d\right)_{+}\right].
%\end{equation}
...
% Each choice of $(f_N,f_U)$ in the tests below has been chosen so $\Exp[S_N]=\Exp[N]\Exp[U]=1$. This does not lose any generality; say the $(f_N,f_U)$ pair of interest has mean $\Exp[S_N]=\mu\not=1$, then we can approximate the $(f_N,f_V)$ pair which is the normalized version
% \[ S_N^* = \frac{S_N}{\mu} = \sum_{i=1}^N \frac{U_i}{\mu} = \sum_{i=1}^N V_i \]
% where $f_V(x) = f_U(x/\mu)/\mu$, and then $\Ftail_{S_N}(\mu x) = \Ftail_{S_N^*}(x/\mu)$.
...
$a$                   & 0.5  & 1  & 1.5  & 2 & 2.5  \\
\hline
% Laplace Inversion survival function & 7.27e-7 & 1.92e-6 & 5.86e-6 & 1.78e-5 & 4.01e-5 \\
...
% \[ \Ftail_U(x) = \bigl( 1 + \frac{x-\theta}{a} \bigr)^{-b} = \bigl( 1 + \frac{x}{5} \bigr)^{-11} \,. \]
...
% \begin{test}
% $\lambda=2$, $U\sim\ExpDist(4)$ and $c=7.2$
% \end{test}
