...
f(S_N)=\min[(S_N-a)_{+},b], \quad b\geq0,
\end{equation}
...
f(S_N)=c(S_N-a)_{+}, \quad 0\leq c\leq1,
\end{equation*}
...
S(t) = \sum_{k=1}^{N(t)} U_k - ct,\quad t\geq0,
\end{equation*}
specifically,
\begin{equation*}
...
Just as earlier, we can generate the polynomials by the Gram--Schmidt procedure if $w$ admits moments of all orders (i.e.\ $\forall n \in \NZ :$ $\Exp[\abs{X}^n] < \infty$ where $X \sim w$), and the resulting polynomials are complete in $\Lp^2(\RL, w(x) \dd x)$ if $\int_{\RL} \e^{s\abs{x}} w(x) \dd x < \infty$ for some $s>0$.
...
=\sum_{i=0}^{n} \frac{\Gamma(n+r)}{\Gamma(n-i+1)\Gamma(r+i)}\frac{(-x)^i}{i!}, \quad n\geq 0,
\end{equation*}
...
\begin{align*}
b_i
&= \sum_{k=0}^\infty \mathrm{Coefficient}(x^i, a_k  p_k(x))
...
c_i&=\frac{1}{i!}\frac{\dd^{i}}{\dd z^{i}}\mathcal{P}(z)\Big\rvert_{z=0}  =\mathrm{Coefficient}(i, \mathrm{MaclaurinSeries}(\mathcal{P}(z))) \, \quad i \in \NZ\,.
\end{align*}
...
\begin{equation*}
a_{k}=\int_{0}^{\infty}p_k(x)f_{S_N}^+(x)\dd x,\quad k\geq0.
\end{equation*}
...
\begin{align*}\label{eq:PolynomialExpansionCoefficientDerivativeGeneratingFunction}
a_k&=\frac{1}{d_k}\frac{1}{k!}\frac{\dd^{k}}{\dd z^{k}}\mathcal{Q}(z)\Big\rvert_{z=0}  =\frac{1}{d_k} \mathrm{Coefficient}(k, \mathrm{MaclaurinSeries}(\mathcal{Q}(z))).
\end{align*}
...
f_{\mathrm{approx}}(x) \defeq \sum_{k=0}^{M_1} \binom{M_1}{k} 2^{-M_1} s_{M_2+k}(x) \approx f(x) \,.
\end{equation}
...
\begin{align*}
\mathrm{ApproximateAbsoluteError}( \widehat{f}_i, x )
&\defeq \widehat{f}_i(x)- \mathrm{Median}\big\{ \widehat{f}_1(x),\dots,\widehat{f}_I(x) \big\} \\
&\approx \widehat{f}_i(x)-f(x) \defeqr \mathrm{AbsoluteError}( \widehat{f}_i, x ) \,.
\end{align*}
